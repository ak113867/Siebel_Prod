if (typeof(SiebelAppFacade.VHASalesCalculatorViewPR) === "undefined") {
    ...
    define("siebel/custom/VHASalesCalculatorViewPR", ["siebel/viewpr", "siebel/custom/VHAAppUtilities", "siebel/custom/bootstrap.min", "siebel/custom/VHASalesCalculatorTemplate", "siebel/custom/VHABTProcessCall"], function () {
        SiebelAppFacade.VHASalesCalculatorViewPR = (function () {
            ...
            var sSession = "";
			var mspProdName="";
            var sRelSwitch = VHAAppUtilities.GetPickListValues("", "[List Of Values.Type]='VHA_RELEASE_SWITCH' AND [List Of Values.Name]='RELEASE_ONE_DOT_TWO' AND [List Of Values.Active]='Y'", "")[0];
            var MspCnt = "";
			var targetMspPlanPropSet,targetMspPlanInpSet;
			var targetPlanMspEligible="N";
            var AccountMaxMspSequence = "";
            var SessionCurrMspSequence = "";
            var MspExistingCust = "N";
            var MspEligible = "N";
            ...
            VHASalesCalculatorViewPR.prototype.ShowUI = function () {
				...
				//18062025:Soumalya:CM-3887:Added code to remove hard-coding product names.
				var prodNameInpSet = SiebelApp.S_App.NewPropertySet();
				prodNameInpSet.SetProperty("FieldName","Name;Siebel Product Type;Version Status");
				prodNameInpSet.SetProperty("Delimeter",";");
				prodNameInpSet.SetProperty("SearchExp","[Siebel Product Type]='MSP Discount' AND [Version Status]='Active'");
				prodNameInpSet.SetProperty("BusCompName","Internal Product");
				prodNameInpSet.SetProperty("BusObjectName","Internal Product");
				var prodNameOutSet = VHAAppUtilities.CallBS("VHA Generic Utilities BS", "QueryBusComp", prodNameInpSet);
				if(prodNameOutSet.PropertyExists("Name"))
				{
					mspProdName = prodNameOutSet.propArray["Name"];
				}
            };
                    ...
                    if (sFilterTxt === "Recommendations" && nestedData.length === 0) {
                        ...
                        if (HTTPErrorCode === '200') {
                            if (ResponseFromAPI.length > 0) {
                                ...
                                const grouped = ResponseFromAPI.reduce((acc, {
                                            propArray
                                        }) => {
                                        const {
                                            InteractionID,
            ...
            function refreshCart(priPlanDtl) {
                ...
                Inputs.SetProperty("RatePlanSAMId", currentRLI.PlanItem.Code);
                Inputs.SetProperty("RatePlanPriceIncGST", currentRLI.PlanItem.Price);
				//11062025:Soumalya:Modified code for MSP sequence for new and existing customer
                if(sRelSwitch == "Y")
				{
					targetPlanMspEligible="N";
					targetMspPlanInpSet = SiebelApp.S_App.NewPropertySet();
					targetMspPlanInpSet.SetProperty("FieldName","Plan Id;Status Flag");
					targetMspPlanInpSet.SetProperty("Delimeter",";");
					targetMspPlanInpSet.SetProperty("SearchExp","[Plan Id]='"+currentRLI.PlanItem.Code+"' AND [Status Flag]='Y'");
					targetMspPlanInpSet.SetProperty("BusCompName","VHA MSP Eligibilty Matrix Active Item BC");
					targetMspPlanInpSet.SetProperty("BusObjectName","VHA MSP Admin BO wo link");
					targetMspPlanPropSet = VHAAppUtilities.CallBS("VHA Generic Utilities BS", "QueryBusComp", targetMspPlanInpSet);
					if(targetMspPlanPropSet.PropertyExists("Status Flag"))
					{
						if(targetMspPlanPropSet.propArray["Status Flag"]=="Y")
							targetPlanMspEligible = "Y";
					}
					if (scJson.QuoteHeader.CustomerType == "Existing") 
					{
						MspExistingCust = "Y";
						if(targetPlanMspEligible == "Y")
						{
							if(scJson.QuoteHeader.QuoteJourney == "Upgrade" && MspEligible == "Y" && targetPlanMspEligible == "Y")
							{
								//continue as is seq
								Inputs.SetProperty("MspCnt", aUpgradeEligibity.MspSequence);
							}
							else
							{
								AccountMaxMspSequence = 0;
								if(scJson.QuoteHeader.ExistingCustDtls.AccountMaxMspSequence)
									AccountMaxMspSequence = parseInt(scJson.QuoteHeader.ExistingCustDtls.AccountMaxMspSequence);
								if(SessionCurrMspSequence == "")
									SessionCurrMspSequence = parseInt(AccountMaxMspSequence);
								else
								{
									if(parseInt(AccountMaxMspSequence)>=parseInt(SessionCurrMspSequence))
										SessionCurrMspSequence = parseInt(AccountMaxMspSequence);							
								}
								SessionCurrMspSequence++;
								MspCnt = SessionCurrMspSequence;
								Inputs.SetProperty("MspCnt", MspCnt);
							}						
						}																	
					}
					else
            ...
            function ProcessOfferResponse(resultSet, callFrom) {
                ...
                if (sRelSwitch == "Y" && resultSet.propArray["BundleSave"] == "MSP Discount") {
                    /*if (resultSet.propArray["BundleSavePercent"] == 0) {
                        MspCnt = MspCnt + 1;
                    } else {
                        var MSP = {};
                        MSP.Action = "Add";
                        MSP.Type = "Bonus";
                        MSP.Price = parseFloat(resultSet.propArray["BundleSavePercent"]).toFixed(2);
                        MSP.UI_Type = "MSPDiscount";
                        MSP.Name = "Bundle and Save";
                        currentRLI.BonusItem.push(MSP);
                        if (MspExistingCust == "N") {
                            MspCnt = MspCnt + 1;
                        }
                    }
					*/
					if (!(resultSet.propArray["BundleSavePercent"] == 0)){
						...
						MSP.Price = parseFloat(resultSet.propArray["BundleSavePercent"]).toFixed(2);
						MSP.UI_Type = "MSPDiscount";
						MSP.Name = mspProdName;//18062025:Soumalya:CM-3887:Changed hard-coded to product name dynamic.
						currentRLI.BonusItem.push(MSP);
					}
                }
            ...
            function getExistingCustomerDetails(callfrom) {
                ...
                case "searchButton":
                    ...
                    if (custmerPin == cusT || sSession == "Resume" || SiebelApp.S_App.GetProfileAttr("SalesCalcExistCust") == "Y") { //Verify PIN
						...
						//11062025:Soumalya:Added for R2.0 MSP
                        scJson.QuoteHeader.ExistingCustDtls.MspSequence = outtmp.GetChildByType("ResultSet")["propArray"]["MspSequence"];
                        scJson.QuoteHeader.ExistingCustDtls.AccountMaxMspSequence = outtmp.GetChildByType("ResultSet")["propArray"]["AccountMaxMspSequence"];
            ...
            function getExistingSeledtedServiceDetails(sAssetId) {
                ...
                if (ExistingContract.length > 0) {
                    //11062025:Soumalya:Modified Code for existing service Details MSP
					if (sRelSwitch == "Y") {
                        if (aUpgradeEligibity.MspSequence > 0) {
                            MspEligible = "Y";
							if (ExistingContract[0]["MultiServiceProposition"] && parseFloat(ExistingContract[0]["MultiServiceProposition"]) > 0)
								selService.ExistingContractUI.BundleandSave = ExistingContract[0]["MultiServiceProposition"];
						}
						else
							MspEligible = "N";
                    } else
            ...
            function DisplayContractDetails(sAssetId, callfrom) {
                ...
                case "ExtCustView":
                    ...
                    var Offershtml = "";
                    if (sRelSwitch == "Y") {
						if(parseFloat(selService.ExistingContractUI.BundleandSave)>0)
							Offershtml = '<div class="vha-sc-currentPlan d-flex mt-2"><div class="col-1"></div><div class="col-7"><li>Early Upgrade Fee<span>' + '$' + selService.ExistingContractUI.EarlyUpgradeFee + '</span></li></div></div><div class="vha-sc-currentPlan d-flex mt-2"><div class="col-1"><span class="vha-sc-BundleAndSave_Icon"></span></div><div class="col-7"><li>Loyalty<span>' + selService.ExistingContractUI.LoyaltyDiscount + '</span></li><li>Bundle & Save<span>' + '$' + selService.ExistingContractUI.BundleandSave + '</span></li><li>Credit<span>' + selService.ExistingContractUI.Credit + '</span></li></div><div class="col-4"></div></div>';
						else
							Offershtml = '<div class="vha-sc-currentPlan d-flex mt-2"><div class="col-1"></div><div class="col-7"><li>Early Upgrade Fee<span>' + '$' + selService.ExistingContractUI.EarlyUpgradeFee + '</span></li></div></div><div class="vha-sc-currentPlan d-flex mt-2"><div class="col-1"><span class="vha-sc-BundleAndSave_Icon"></span></div><div class="col-7"><li>Loyalty<span>' + selService.ExistingContractUI.LoyaltyDiscount + '</span></li><li>Credit<span>' + selService.ExistingContractUI.Credit + '</span></li></div><div class="col-4"></div></div>';
                    } else
                        Offershtml = '<div class="vha-sc-currentPlan d-flex mt-2"><div class="col-1"></div><div class="col-7"><li>Early Upgrade Fee<span>' + '$' + selService.ExistingContractUI.EarlyUpgradeFee + '</span></li></div></div><div class="vha-sc-currentPlan d-flex mt-2"><div class="col-1"><span class="vha-sc-BundleAndSave_Icon"></span></div><div class="col-7"><li>Loyalty<span>' + selService.ExistingContractUI.LoyaltyDiscount + '</span></li><li>Bundle & Save<span>' + selService.ExistingContractUI.BundleandSave + '</span></li><li>Credit<span>' + selService.ExistingContractUI.Credit + '</span></li></div><div class="col-4"></div></div>';
                ...
                case "ExtCartSummary":
                    ...
                    var Offershtml1 = "";
                    if (sRelSwitch == "Y") {
                        Offershtml1 = '<div class="vha-sc-other vha-sc-planDesign"><span></span><div>Early Upgrade Fee<br><button class="vha-sc-EarlyUpgradeBtn" id="vha-sc-EarlyUpgradeBtn">12</button><button class="vha-sc-EarlyUpgradeBtn" id="vha-sc-EarlyUpgradeBtn">24</button><button class="vha-sc-EarlyUpgradeBtn applet-button-active" id="vha-sc-EarlyUpgradeBtn">One Time</button></div><div class="vha-sc-alignSpanDiv"><br><br><span>' + '$' + selService.ExistingContractUI.EarlyUpgradeFee + '</span><br><br>&nbsp;</div></div><div class="vha-sc-other vha-sc-planDesign"><span></span><div>Loyalty<br>Credit</div><div class="vha-sc-alignSpanDiv"><span>' + selService.ExistingContractUI.LoyaltyDiscount + '</span><br><span>' + selService.ExistingContractUI.Credit + '</span></div></div>';
                    } else
            ...
            function setUI() {
                ...
                for (var a = 0; a < tsPlantype.length; a++) {
                    ...
                    /*	var pegaResp = VHAAppUtilities.GetPickListValues("", "[List Of Values.Type]='VHA_FUNCTION_ACCESS_RESP' AND [List Of Values.Value]= 'Resp|PegaResp' AND [List Of Values.Active]='Y'", {
                    "All": "true"
                    })[0].Name;
                    ...
                    if (sLovFlg == "N" && tsPlantype[a].Id == "vhascRecommendplan") {}
                    else {
            ...
            var selectAutoCompleteVal = function (e, u) {
                ...
                case "Accessory":
                    ...
                    $("#sc-accname-search").autocomplete({
                        source: removeDuplicates(accessoryresult.filter(item => item.category === u.item.value), 'value').map(item => ({
                                label: item.value,
                                value: item.value,
                                type: "AccessoryName",
                                prodcd: item.prodcd
            ...
            function TriggerNBNAddress(sResp, NBNLoc) {
                ...
                function appendDialogTemplate() {
                    ...
                    $("#openModal").delegate(".VHASelectNBNAddress", "click", {
                        ctx: this
                    }, processSelectNBNAddressSelect);
                }
