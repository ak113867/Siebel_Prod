if (typeof SiebelAppFacade.VHAAccessoriesPR === "undefined") {
    ...
    define("siebel/custom/VHAAccessoriesPR", [
        ...
        SiebelAppFacade.VHAAccessoriesPR = (function () {
                    ...
                    if (listOfassetHd.GetChildCount() > 0) {
                        ...
                        for (var a = 0; a < l; a++) {
                            $($($(".VHABillPick"))[a]).off("click").on("click", function () {
								...
								if(sRelSwitch == "Y")
								{
									var sTotId = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VHA TOT Generic BC").GetFieldValue("Id");
									var sBanId = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VHA TOT Generic BC").GetFieldValue("Billing Account Id");
									if(sBanId == "" || sBanId == null){sBanId = this.id;}
									
									var sPlName = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VHA TOT Generic BC").GetFieldValue("Plan Name");
									var Inp = SiebelApp.S_App.NewPropertySet();
									var Out = SiebelApp.S_App.NewPropertySet();
									Inp.SetProperty("Object Id", sTotId);
									Inp.SetProperty("BillActId", sBanId);
									Inp.SetProperty("PlanName", sPlName);
									Out = VHAAppUtilities.CallWorkflow(
									"VHA TOS MSP Discount Calc WF",
									Inp);
									VHAAccessoriesPR.initializeMSPDiscount(Out.propArray.MSPDiscount);
								}
                                else VHAAccessoriesPR.initializeBundleSave();
			...
			VHAAccessoriesPR.initializeMSPDiscount =  function (MSPDiscount) {
				if(MSPDiscount > 0){
                var MspObj = {
				    "type": "cart-discount-msp",
                    "subType": "cart-discount-msp",
                    "dollar": MSPDiscount,
                    "value": MSPDiscount
                };
                SiebelAppFacade.VHAToTCommonViewPR.addCartItems(MspObj);
				}
            };
			VHAAccessoriesPR.initializeSpeedBoost =  function (sSBProduct,sSBSpeed) {
				 var SBObj = {
				    "type": "vha-tot-SBplan",
                    "subType": "vha-tot-SBplan",
                    "dollar": sSBProduct,
                    "value": sSBSpeed
                };
                SiebelAppFacade.VHAToTCommonViewPR.addCartItems(SBObj);
				};
