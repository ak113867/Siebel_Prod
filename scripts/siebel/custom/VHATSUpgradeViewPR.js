if (typeof SiebelAppFacade.VHATSUpgradeViewPR === "undefined") {
    ...
    define("siebel/custom/VHATSUpgradeViewPR", ["siebel/viewpr", "siebel/custom/VHAAppUtilities", "siebel/custom/bootstrap.min", "siebel/custom/VHATSUpgradeTempalte", "siebel/custom/VHABTProcessCall"], function () {
        SiebelAppFacade.VHATSUpgradeViewPR = function () {
            var vDispMSP = "";
            var vPdfMSP = "";
            var vBonusMSPPS = null;
            var vMSPReleaseOneDotTwoSwitch = "N";
            var vMSPHeaderDetail = "";
            var sOrderRef;
            ...
            VHATSUpgradeViewPR.prototype.ShowUI = function () {
                ...
                showCustomerDtls();
                setUI();
                //16012025:Soumalya:Added code to get the R1.2 switch value
                ...
                $.ajax(conSettings).done(function (response) {
                    if (response["Error Code"] == "") {
                        ...
                        tsintitialData.OrderHeader.Plan = plan;
						earlyupgradeFee();
                        ...
                        if (vMSPReleaseOneDotTwoSwitch == "Y") {
                            $(".e-cart-discount-bundle").addClass("VFDisplayNone");
                            $(".e-cart-discount-msp .vha-ts-cartval span").text(currentOffers["MultiServiceProposition"]);
                            if (currentOffers["MultiServiceProposition"] != "" && currentOffers["MultiServiceProposition"] > 0)
                                $(".e-cart-discount-msp").removeClass("VFDisplayNone");
                            else
                                $(".e-cart-discount-msp").addClass("VFDisplayNone");
                        } else {
                            $(".e-cart-discount-msp").addClass("VFDisplayNone");
                            $(".e-cart-discount-bundle").removeClass("VFDisplayNone");
                            $(".e-cart-discount-bundle .vha-ts-cartval span").text(currentOffers["BundleandSave"]);
                        }
				...
				function earlyupgradeFee()
					{
						...
						var earlyUpgradeFee = Result.childArray[0].childArray[0].childArray[0].childArray[1].childArray[0].propArray['EarlyUpgradeFee'];
						$("#vha-ts-cart-e-upg-fee").text("$ " + earlyUpgradeFee);
						tsintitialData.OrderHeader.ETC = earlyUpgradeFee;
						if ($("#vha-ts-cart-e-upg-fee").text() != "$ 0.00") {
							$(".vha_ts_earlyupfee").attr("disabled", false);
						} else {
							if ($("#vha-ts-cart-e-upg-fee").text() == "$ 0.00") {
            ...
            VHATSUpgradeViewPR.prototype.BindEvents = function () {
                                ...
                                for (let i = 0; i < filteredarray.length; i++) {
                                    for (let j = 0; j < sRsnBasedplnresp.length; j++) {
                                        if (sRsnBasedplnresp[j]._source.Plan_Code.toLowerCase() === filteredarray[i].SAM_Product_ID.toLowerCase()) {
											sRsnBasedplnresp[j].MSISDN = filteredarray[i].ContextID;
                ...
                $("#vha-ts-d-carousel-container").off("change").on("change", "select#vhatscolour", function () {
					...
					var termValuecolor = $(".vhats-d-term.applet-button-active").attr("term");
                    for (i = 0; i < filtereddevicesresponse.length; i++) {
                        if (filtereddevicesresponse[i]._source.Source_Product_Name == sDviceName && filtereddevicesresponse[i]._source.Capacity == $(this).parent().find("select#vhatscapacity").val() && filtereddevicesresponse[i]._source.Color == $(this).val()) {
                            $(this).parent().parent().next().find("[id^=vha-ts-d-amt]").text(mTruncate(parseFloat(filtereddevicesresponse[i]._source.RRP_inc_gst / termValuecolor).toString()));
                ...
                $("#vha-ts-d-carousel-container").on("change", "select#vhatscapacity", function () {
					...
					var termValuemonth = $(".vhats-d-term.applet-button-active").attr("term");
                    for (i = 0; i < filtereddevicesresponse.length; i++) {
                        if (filtereddevicesresponse[i]._source.Source_Product_Name == sDviceName && filtereddevicesresponse[i]._source.Capacity == $(this).val() && filtereddevicesresponse[i]._source.Color == $(this).parent().find("select#vhatscolour").val()) {
							...
							$(this).parent().parent().next().find("[id^=vha-ts-d-amt]").text(mTruncate(parseFloat(filtereddevicesresponse[i]._source.RRP_inc_gst / termValuemonth).toString()));
                            $(this).parent().parent().next().find("[id^=vha-ts-d-desc] span").text(filtereddevicesresponse[i]._source.RRP_inc_gst);
                            $(this).parent().parent().next().find(".vha-ts-d-shopaddtocartbtn").attr("price", filtereddevicesresponse[i]._source.RRP_inc_gst);
                        }
                    }
                ...
                $("#smartwizard").on("vhacustomBtnClick", function (evt, btn) {
                    ...
                    case "vha-cancel-btn":
                        ...
                        break;
                    case "vha-tot-finish":
                        var NewPlan = tsintitialData.OrderHeader.RedPlusPlan.Name;
                        var Oldplan = tsintitialData.OrderHeader.Plan.CurrentPlan;
                        var sdev = tsintitialData.OrderHeader.Device.Device__Name;
                        var sDetsBC = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VHA 3 Step Upgrade Header");
                        var sOrderId = sDetsBC.GetFieldValue("Order Id");
                        if (NewPlan == Oldplan && sdev == "") {
                            var ser = SiebelApp.S_App.GetService("VF BS Process Manager");
                            var Inpp = SiebelApp.S_App.NewPropertySet();
                            Inpp.SetProperty("Service Name", "VHA 3 Step Update Contract Term");
                            Inpp.SetProperty("Method Name", "GetContractDetails");
                            Inpp.SetProperty("OrderId", sOrderId);
                            Inpp.SetProperty("New Plan", NewPlan);
                            Inpp.SetProperty("Plan Term", sPlanTerm);
                            var outp = ser.InvokeMethod("Run Process", Inpp);
                        }
                        TerminateValueCart = 0;
                        pegaflag = "N";
                        $("#maskoverlay").styleShow();
                                                    ...
                                                    if (resultdata == false) {
                                                        ...
                                                        if (validateSharingDetails() == "ERROR_SELECT_MSISDN") {
                                                            ...
                                                            if (prepaymentValidation() == false) {
                                                                ...
                                                                //16010225:Soumalya:Added code for MSP R1.2 Start
                                                                if (vMSPReleaseOneDotTwoSwitch == "Y") {
                                                                    vPdfMSP = tsintitialData.OrderHeader.DeviceDiscounts.MultiServiceProposition;
                                                                    if (vPdfMSP != "" && Number(vPdfMSP) > 0) {
                                                                        vBonusMSPPS = SiebelApp.S_App.NewPropertySet();
                                                                        vBonusMSPPS.SetType("Credit");
                                                                        vBonusMSPPS.SetProperty("Name", "Bundle and Save");
                                                                        vBonusMSPPS.SetProperty("Term", "Month to Month");
                                                                        vBonusMSPPS.SetProperty("DiscountPrice", "$" + vPdfMSP);
                                                                        resultset.GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChildByType("ListOfCredit").AddChild(vBonusMSPPS.Clone());
                                                                    }
                                                                    //02062025:Soumalya:Added new line to set the header detail.
                                                                    resultset.GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0).SetProperty("MSPHeaderDetail", vMSPHeaderDetail);
                                                                }
                                                                //16010225:Soumalya:Added code for MSP R1.2 End
                                                                ...
                                                                var OrderItems;
                                                                sPlanTerm = ServiceOrder.GetProperty("PlanTerm");
            ...
            function mspTruncate(sValue) {
                //16012025:Soumalya:Added new method to truncate the MSP discount amount to 2 decimal point
                var sPoint = "",
                sTruncValue = "0.00",
                sLeft = "0",
                sRight = "00";
            ...
            function pegaOrderValidation() {
				...
				if(VHAAppUtilities.GetPickListValues("", "[List Of Values.Type]='VF_CR_ENABLE_FLAG' AND [List Of Values.Name]='ENABLE_CMID_SIMO'")[0] == 'Y')
				{
					psInputs.SetProperty("PlanType", pegaoffData.PlanType);
				}
            ...
            function totalIndicativeCostCalc() {
                ...
                if (tsintitialData.OrderHeader.DeviceDiscounts.MultiServiceProposition)
                    sTotalIndCost -= parseFloat(parseFloat(tsintitialData.OrderHeader.DeviceDiscounts.MultiServiceProposition).toFixed(2));
                if (sTotalIndCost > 0) {
            ...
            function addDiscounts(sDvcIMEIChk) {
                ...
                $(".vha-ts-discount, .cart-discount-msp").addClass("VFDisplayNone");
                ...
                //16012025:Soumalya:Added new switch based code block for MSP R1.2
                if (vMSPReleaseOneDotTwoSwitch == "Y") {
                    if (sDvcIMEIChk["CurrAssetSeq"] != "" && sDvcIMEIChk["CurrAssetSeq"] != null) {
                        vMSPHeaderDetail = "Eligible";
                        if (sDvcIMEIChk["CurrAssetSeq"] == "1")
                            vMSPHeaderDetail = "Primary Service";
                    }
					else {
						vMSPHeaderDetail = "NA";
						if(sDvcIMEIChk["PropSiebProdType"] == "Voice")
							vMSPHeaderDetail = "Ineligible";
					}
                    sDvcDisc.MultiServiceProposition = "";
                    if (sDvcIMEIChk["MultiServiceProposition"] != "" && parseFloat(parseFloat(sDvcIMEIChk["MultiServiceProposition"]).toFixed(2)) > 0) {
                        $(".vha-ts-discount, .cart-discount-msp").removeClass("VFDisplayNone");
                        $(".cart-discount-msp .vha-ts-cartval span").text(mspTruncate(sDvcIMEIChk.MultiServiceProposition));
                        sDiscounts += parseFloat(parseFloat(sDvcIMEIChk["MultiServiceProposition"]).toFixed(2));
                        sDvcDisc.MultiServiceProposition = mspTruncate(sDvcIMEIChk.MultiServiceProposition);
                    }
                } else {
            ...
            function setUI() {
                ...
                for (var a = 0; a < tsPlantype.length; a++) {
                    ...
                    if (sLovFlg == "N" && tsPlantype[a].Id == "vhatsRecommendplan") {}
                    else {
            ...
            function createPlanTiles(sRsnBasedplnresp) {
                $("#vha-ts-p-carousel-container").off("click").on("click", ".vha-ts-addtocartbtn", function () {
                    ...
                    tssleep(30).then(() => {
						...
						var sPlanmsisdn = $(this).attr("planmsisdn");
                        ...
                        if (Pegafilteredarray != undefined && pegaflag === "Y") {
                            for (let i = 0; i < Pegafilteredarray.length; i++) {
                                if (Pega_sel_PlanSAMID.toLowerCase() === Pegafilteredarray[i].SAM_Product_ID.toLowerCase() && 
								sPlanmsisdn == Pegafilteredarray[i].ContextID) {
                        ...
                        sErrMsg = "",
                        ROups = "",
                        sVisaErrorFlag = ""; //TULASIY::08-07-2023::Added sVisaErrorFlag for Apple Voucher Validation
