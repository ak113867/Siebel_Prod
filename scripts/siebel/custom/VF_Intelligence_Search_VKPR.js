if (typeof(SiebelAppFacade.VF_Intelligence_Search_VKPR) === "undefined") {
 ...
 $('[aria-label="RAF Status"]').addClass("VFDisplayNone");
            ...
            function VHACovergaeCheck(sResp, this_t) { // ??
                if (sResp == "") {
                    ...
                    var sOut = VHAAppUtilities.CallBS("VHA FWA Store Pickup Reservation Service BS MOCN", "GetAddressId", nInputs, "")
						...
						ui.item.lat = sOut.propArray.Latitude;
						ui.item.lon = sOut.propArray.Longitude;						
                    var sResp = VHAAppUtilities.getAddress(ui);
					}
                }
                if (sResp != "" || (ui.item.lat != "" && ui.item.lon != ""))
					fCoverageCheck(sResp, this_t, ui);
            ...
            function fCoverageCheck(sResp, this_t, ui) {
				...
				//{
					...
					if(sResp != "")
					{
						...
						nInputs.SetProperty("GNAFId", sResp.address.id);
					}
					else
					{
						SiebelApp.S_App.SetProfileAttr("Longitude",ui.item.long);
						SiebelApp.S_App.SetProfileAttr("Latitude",ui.item.lat);
						nInputs.SetProperty("longitude", ui.item.lon);
						nInputs.SetProperty("latitude", ui.item.lat);	
						nInputs.SetProperty("GNAFId", ui.item.id);					
					}					
					if(SiebelApp.S_App.GetProfileAttr("InvokeFWAConnect") == "Yes" && (SiebelApp.S_App.GetProfileAttr("fwaSqId5G") != "" || SiebelApp.S_App.GetProfileAttr("fwaSqId4G") != ""))
						nInputs.SetProperty("Service Name", "VHA IM FWA Store Pickup Reservation Service BS MOCN");
					else 
					...
					if(SiebelApp.S_App.GetProfileAttr("InvokeFWAConnect") == "Yes")
					{						
						nInputs.SetProperty("GNAFId", SiebelApp.S_App.GetProfileAttr("SwitchToFWAGnafId"));
						SiebelApp.S_App.SetProfileAttr("SwitchToFWAGnafId","");
					}
					var sView = SiebelApp.S_App.GetActiveView().GetName();
					if (sView == "VF Capture Customer Details Postpay UNISIM TBUI") {                    
